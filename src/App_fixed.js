...
Prêt à vous connecter? Envoyez-moi un message!`
    ...
    // FIX 1: Limpiar timeouts al desmontar
    useEffect(() => {
        return () => {
    ...
    // FIX 1: Mejorar la secuencia de construcción para evitar bucles
    const startBuildingSequence = () => {
        ...
        const interval = setInterval(() => {
            ...
            if (progress >= 100) {
                ...
                // FIX: Asegurar que solo se ejecute una vez
                buildingTimeoutRef.current = setTimeout(() => {
                    console.log('✅ Finalizando construcción - FIXED');
                    setIsBuilding(false);
                    setBuildingProgress(0);
                    // Limpiar la referencia
                    buildingTimeoutRef.current = null;
                }, 1500);
            }
        }, 600);
        ...
        // Fallback mejorado
        buildingTimeoutRef.current = setTimeout(() => {
            console.log('🔧 Fallback activado - finalizando construcción');
            clearInterval(interval);
            setIsBuilding(false);
            setBuildingProgress(0);
            buildingTimeoutRef.current = null;
    ...
    // Building Effect - VERSIÓN MEJORADA SIN BUCLES
    useEffect(() => {
        ...
        const draw = () => {
            ...
            // FIX: Detener después de 3.5 segundos o si isBuilding es false
            if (elapsed > 3500 || !isBuilding) {
                isAnimating = false;
    ...
    return (
        <div className="portfolio-container bg-black text-green-400 font-mono flex flex-col relative">
            ...
            {/* Main Layout - FIX 2: Cambiar estructura para permitir scroll */}
            {!isBuilding && (
                <div className="main-layout">
                    {/* Desktop Sidebar Navigation */}
                    <div className="hidden md:flex w-72 bg-black bg-opacity-80 backdrop-blur-sm border-r border-green-500 p-4 flex-col relative z-20 shadow-2xl fixed left-0 top-0 bottom-0">
                    ...
                    {/* Main Content Area - FIX 2: Ajustar para permitir scroll */}
                    <div className="content-wrapper">
                        <div className="content-scroll">
                            <div className="max-w-5xl w-full mx-auto text-center py-8 px-4 md:px-6">
                        ...
                        {/* Terminal Section - FIX 2: Posición ajustada */}
                        {showTerminal && (
            ...
            {/* Custom Styles - FIX 2: Estilos actualizados para scroll */}
            <style jsx>{`
                .portfolio-container {
                    min-height: 100vh;
                    position: relative;
